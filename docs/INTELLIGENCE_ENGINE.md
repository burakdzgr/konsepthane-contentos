...
| **EVIDENCE** | `ResearchEvidence` statements with the full provenance chain to a fetch snapshot: deterministic metadata observations plus model-assisted statements that are each pinned to a verbatim excerpt of the normalized text (rejected when the excerpt cannot be located). Only sources whose role permits it produce evidence; packs need two independent sources, linked by topic across sources. | AI output without an excerpt. Community posts. Model pretraining. |
